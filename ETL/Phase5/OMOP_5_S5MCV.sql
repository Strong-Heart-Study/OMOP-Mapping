...
update #vbles set units = 'fL', unit_concept_id = 8583 where vble = 'S5MCV'


---select * from omop1.dbo.measurement where unit_concept_id=8583 and  measurement_source_value= 'S5MCV'
...
begin transaction t1
	...
	select 'S5MCV', 0, 'SHS', 3024731, 'LOINC', '1970-01-01',	'2099-12-31'-- union all 
...
IF EXISTS (SELECT name FROM sys.sequences WHERE name = N'measurement_seq')
	ALTER SEQUENCE dbo.measurement_seq RESTART WITH 934186 INCREMENT BY 1  
ELSE 
	CREATE SEQUENCE dbo.measurement_seq  START WITH 934186 INCREMENT BY 1;	
;  
GO 
...
----testing 
	...
	-- source
	select 'V' Phase, 'S5MCV' [vble], S5MCV, count(*) from omoprawdata.dbo.S5ALL23 where S5MCV is not null group by S5MCV order by phase, S5MCV
