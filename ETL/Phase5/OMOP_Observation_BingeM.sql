...
select next Value for  omop1.dbo.observation_seq  as [observation_id]
      ...
      , 4145080                             as [qualifier_source_value] -- Frequency per month
...
from (
      ...
      ,case 
            when pat.S5BINGEM = 0 then 'S5BINGEM = 0 No'
            when pat.S5BINGEM = 1 then 'S5BINGEM = 1 binge drinker'
            when pat.S5BINGEM = 2 then 'S5BINGEM = 2 heavy binge drinker'
      end                          as observation_source_value
      , pat.S5BINGEM               as value_as_number
       ,case 
            when pat.S5BINGEM = 0 then 4188540 -- no
            when pat.S5BINGEM = 1 then 4027367 -- binge
            when pat.S5BINGEM = 2 then 4336673 -- heavy drinker
      end 
