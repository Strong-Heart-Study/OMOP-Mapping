...
--------------------------------------------------
	...
	from omop1.dbo.source_to_concept_map sc
		...
		where source_code  in ( 'S5DMHX', 'S5HTNHX')
...
select next Value for  dbo.observation_seq as observation_id
	...
	-- source
	select 'V' Phase, 'S5DMHX' [vble], S5DMHX, count(*) from omoprawdata.dbo.S5ALL23 where S5DMHX is not null group by S5DMHX 
	...
	-- source
	select 'V' Phase, 'S5HTNHX' [vble], S5HTNHX, count(*) from omoprawdata.dbo.S5ALL23 where S5HTNHX is not null group by S5HTNHX
	...
	and left(vo.visit_source_value, 11) like 'SHS Phase V%' and left(vo.visit_source_value, 11) != 'SHS Phase V'
	group by substring(vo.visit_source_value, 11,  CHARINDEX(' ', vo.visit_source_value, 11)-11), o.observation_source_value
	order by phase, observation_source_value
