...
SELECT IDNO, obs_name, obs_value, 
	cast(S6EXDATE as date)  as obs_date,  
	'Phase VI' as Phase,
	cast(NULL as bigint) as value_concept_id
into #obs
FROM   
   (SELECT [IDNO], convert(datetime, S6EXDATE, 121) S6EXDATE
	, S6DMHX
	, S6HTNHX
...
--add the Observation value as concept id 
update mg 
...
-- add concept id mapping into source_to_concept_id 
insert into omop1.dbo.source_to_concept_map (source_code, source_concept_id, source_vocabulary_id, source_code_description, target_concept_id
	, target_vocabulary_id, valid_start_date, valid_end_date, invalid_reason)
select 'S6DMHX', 0,	'SNOMED', 'SHS5 DM HISTORY-SELF REPORTED, Y,N,DURING PREGNANCY,UNKNOWN',	4058709,	'LOINC',	'1970-01-01',	'2099-12-31',	NULL
union all 
select 'S6HTNHX',	0,	'SNOMED',	'SHS1 HTN HISTORY,Y,N',	4058286,	'LOINC',	'1970-01-01',	'2099-12-31',	NULL	
