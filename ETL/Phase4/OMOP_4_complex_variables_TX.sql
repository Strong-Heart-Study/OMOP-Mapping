...
begin 
	...
	insert into omop1.dbo.source_to_concept_map ( source_code, source_concept_id, source_vocabulary_id, target_concept_id, target_vocabulary_id
		, valid_start_date, valid_end_date)
			...
			--standard concepts
	--21600712	DRUGS USED IN DIABETES	Drug	ATC	ATC 2nd	C	A10	1970-01-01	2099-12-31	NULL 
	select 'S4DMTX_B', 0, 'SHS', 21600712, 'ATC', '1970-01-01',	'2099-12-31' union all -- condition mapping (standard concept)
	select 'S4DMTX_I', 0, 'SHS', 21600712, 'ATC', '1970-01-01',	'2099-12-31' union all -- condition mapping (standard concept)
	select 'S4DMTX_O', 0, 'SHS', 21600712, 'ATC', '1970-01-01',	'2099-12-31' union all -- condition mapping (standard concept)

	--21600381	ANTIHYPERTENSIVES	Drug	ATC	ATC 2nd	C	C02	1970-01-01	2099-12-31	NULL
	select 'S4HTNRX2_Y', 0, 'SHS', 21600381, 'ATC', '1970-01-01',	'2099-12-31' union all -- condition mapping (standard concept)

	---- custom concepts
	select 'S4DMTX', 0, 'SHS', 2000000008, 'SHS', '1970-01-01',	'2099-12-31' union all -- observation mapping (standard concept)
	select 'S4HTNRX2', 0, 'SHS', 2000000009, 'SHS', '1970-01-01',	'2099-12-31' -- observation mapping (standard concept)
...
, S4EXDATE																[drug_exposure_start_date]
, cast(S4EXDATE as datetime)											[drug_exposure_start_datetime]
...
where  ( v.vble = 'S4DMTX'	and (v.[value] like '%B%' or v.[value] like '%I%' or v.[value] like '%O%' ))	-- only for pats w/ diabetic treatment
...
left join omop1.dbo.source_to_concept_map c on concat(v.vble, '_', ltrim(rtrim(replace(v.[value], ' ', '')))) = c.source_code and c.target_vocabulary_id != 'SHS'
left join omop1.dbo.visit_occurrence vo1 on vo1.person_id = v.[IDNO] and vo1.visit_source_value like 'SHS Phase IV %'
where  ( v.vble = 'S4HTNRX2' and v.[value] like '%Y%' )	-- only for pats w/ HTN treatment
...
select next Value for  dbo.observation_seq as observation_id
	...
	-- source
	select 'IV' Phase, 'S4DMTX' [vble], S4DMTX, count(*) from omoprawdata.dbo.S4ALL23 where S4DMTX is not null  group by S4DMTX
	...
	group by substring(vo.visit_source_value, 12,  CHARINDEX(' ', vo.visit_source_value, 12)-12), drug_source_value
	...
	-- source
	select 'IV' Phase, 'S4HTNRX2' [vble], S4HTNRX2, count(*) from omoprawdata.dbo.S4ALL23 where S4HTNRX2 is not null  group by S4HTNRX2
