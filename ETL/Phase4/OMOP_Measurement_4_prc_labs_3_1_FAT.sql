...
--Check if S4Fat is already present	
select measurement_source_value, count(*) counts 
from omop1.dbo.measurement 
where measurement_source_value = 'S4FAT'
...
, Phase = 'Phase V'
, Unit = case 
	when Lab_name = 'S4AST' then 'iU/L' 
	when Lab_name = 'S4CRP' then 'mg/L' 
	when Lab_name = 'S4HGB' then 'g/dL' 
	when Lab_name = 'S4INSU' then 'uU/mL' 
	when Lab_name = 'S4PLT' then '10**9/L' 

	when Lab_name = 'S4POT' then 'mEq/L' 
	when Lab_name = 'S4TPROT' then 'gm/dL' 
	when Lab_name = 'S4U_ALB' then 'mg/dL' 
	
	when Lab_name = 'S4FAT' then 'kg' 
	when Lab_name = 'S4UCREA' then 'mg/dL' 
