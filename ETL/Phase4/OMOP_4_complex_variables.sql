...
begin
	...
	select 2000000007,  'DIABETES STATUS ACCORDING TO 1997 ADA CRITERIA', 'Observation', 'SHS', 'SHS', NULL as standard_concept, 'ADADM', '2020-10-01', '2099-12-31'
...
begin 
	...
	--320128	Essential hypertension	Condition	SNOMED	Clinical Finding	S	59621000	1970-01-01	2099-12-31	NULL
	select 'S4USHTN', 0, 'SHS', 320128, 'SNOMED', '1996-09-08',		'2099-12-31' union all -- condition mapping (standard concept)
	select 'S4USHTN', 0, 'SHS', 2000000005, 'SHS', '1996-09-08',	'2099-12-31' union all --  observation mapping (custom concept)


	 --201820	Diabetes mellitus	Condition	SNOMED	Clinical Finding	S	73211009	1970-01-01	2099-12-31	NULL
	select 'S4ADADM', 0, 'SHS', 201820, 'SNOMED', '1996-09-08',	'2099-12-31' union all -- condition mapping (standard concept)
	select 'S4ADADM', 0, 'SHS', 2000000007, 'SHS', '1996-09-08',	'2099-12-31'  --  observation mapping (custom concept)	
		----
...
, S4EXDATE as [condition_start_datetime]
...
where ( v.vble in ( 'S4USHTN') and v.[value] like '%Y%' ) -- only people with value = 'Y' for hypertension
or ( v.vble in ('S4ADADM') and v.[value] like '%DM%' )	-- only diabetic pats
...
select next Value for  dbo.observation_seq as observation_id
	...
	from (
		...
		, cast(S4EXDATE as datetime)					as observation_date		
		, pat.vble
...
-- USHTN
	...
	and left(observation_source_value, 2) = 'S4'
...
--45884153	Normal	Meas Value	LOINC	Answer	S	LA6626-1	1970-01-01	2099-12-31	NULL
	...
	from omop1.dbo.observation o
	left join omop1.dbo.concept c on o.value_as_concept_id = c.concept_id
	where observation_concept_id = 2000000007	--DIABETES STATUS ACCORDING TO 1985 WHO CRITERIA
	and left(observation_source_value, 2) = 'S4'
